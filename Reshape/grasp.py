...
class Open3DGraspVizNode(Node):
    def __init__(self) -> None:
        ...
        self.target_classes = self.get_parameter("target_class").value
        ...
        self.grasp_pub = self.create_publisher(PointStamped, "/grasp", 10)
    ...
    def depth_callback(self, msg: Image) -> None:
        ...
        self.infer_interval = 0.5   # 5Hz（0.2秒一次）
        now = time.time()
    ...
    def color_callback(self, msg: Image) -> None:
        ...
        if target is None:
            self.log_throttled(f"no mask found for class {self.target_classes}")
            with self.data_lock:
        ...
        try:
            ...
            out.point.x = float(point_robot[0])
            out.point.y = float(point_robot[1])
            out.point.z = float(point_robot[2])
    ...
    def find_target_mask_center(
        ...
        for index, class_id in enumerate(classes):
            ...
            if names[class_id] not in self.target_classes:
                continue
