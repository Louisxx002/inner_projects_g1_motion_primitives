...
try:
    ...
    if not hasattr(pin, "SE3") or not hasattr(pin, "Quaternion"):
        raise AttributeError("pinocchio module does not expose SE3/Quaternion")
    HAS_PINOCCHIO = True
    print("Pinocchio imported successfully")
except (ImportError, AttributeError):
    HAS_PINOCCHIO = False
    print("Pinocchio not available or incompatible; using mock SE3/Quaternion")
    # Create a simple mock for SE3 and Quaternion if pinocchio is not available
    class MockSE3:
        def __init__(self, rotation=None, translation=None):
            if hasattr(rotation, "rotation"):
...
from .g1_types import MotionType, Motion
...
class G1Primitives:
    ...
    @staticmethod
    def _as_homogeneous(tf):
        """
        Convert an SE3-like object to a 4x4 homogeneous transform.
        Supports both APIs:
        - obj.homogeneous   (property returning ndarray)
        - obj.homogeneous() (method returning ndarray)
        """
        h = tf.homogeneous
    ...
    def wait_arm_ready(self):
        while not self.arm_controller.arm_ready:
            time.sleep(0.05)
    ...
    def dual_arm_movement(self,
        ...
        sol_q, sol_tauff = self._solve_ik_iterative(
            self._as_homogeneous(L_tf_target),
            self._as_homogeneous(R_tf_target),
            current_q,
            current_dq,
        )
