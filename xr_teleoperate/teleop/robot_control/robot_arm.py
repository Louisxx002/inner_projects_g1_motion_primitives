...
class G1_29_ArmController:
    ...
    def _ctrl_motor_state(self):
        """
        增加腿部初始化，机器人先调整站姿进入预设姿势，再调整上半身进入预设姿势
        """
        if self.motion_mode:
        ...
        self.stand_duration = 10.0
        self.arm_zero_duration = 5.0

        self.leg_waist_indices = list(range(15))
        # 🌟 更自然的站立姿态（需要你微调）
        self.stand_target_q = [0.0] * 15

        # 👉 重点：调腰部 pitch（前后倾）
        # 假设 index 2 是 WaistPitch（你需要确认）
        WAIST_PITCH = 14
        self.stand_target_q[0] = -0.2   # LeftHipPitch
        self.stand_target_q[6] = -0.2   # RightHipPitch

        self.stand_target_q[WAIST_PITCH] = -0.3   # ⭐ 往后仰一点（弧度很关键）
        
        self.official_kp = [200,200,200,200,100,100,200,200,200,200,100,100,150,150,150]
        self.official_kd = [3,3,3,4,2,2,3,3,3,4,2,2,3,3,3]
        # 🌟🌟🌟 结束 🌟🌟🌟
        ...
        while True:
            ...
            # 🌟🌟🌟 新增：在发给底层前，把下半身的平滑站立指令一起塞进数组 🌟🌟🌟
            elapsed_phase = start_time - self.phase_start_time

            # ======================
            # 🌟 Phase 0: 腿归0
            # ======================
            if self.phase == 0:
                ratio = min(1.0, elapsed_phase / self.stand_duration)

                for i in self.leg_waist_indices:
                    target_q = self.stand_target_q[i]
                    current_q = (1.0 - ratio) * self.all_motor_q[i] + ratio * target_q
                    self.msg.motor_cmd[i].q = current_q
                    self.msg.motor_cmd[i].dq = 0.0
                    self.msg.motor_cmd[i].kp = self.official_kp[i]
                    self.msg.motor_cmd[i].kd = self.official_kd[i]
            ...
            # ======================
                ...
                # 腿保持0位（锁住）
                for i in self.leg_waist_indices:
                    self.msg.motor_cmd[i].q = self.stand_target_q[i]
                    self.msg.motor_cmd[i].dq = 0.0
                    self.msg.motor_cmd[i].kp = self.official_kp[i]
                    self.msg.motor_cmd[i].kd = self.official_kd[i]
                    self.msg.motor_cmd[i].tau = 0.0
            ...
            # ======================
                ...
                # 手臂归0
                arm_ratio = min(1.0, elapsed_phase / self.arm_zero_duration)
