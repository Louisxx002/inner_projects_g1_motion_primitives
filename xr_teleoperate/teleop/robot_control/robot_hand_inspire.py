...
class Inspire_Controller_DFX:
    def __init__(self, left_hand_array, right_hand_array, dual_hand_data_lock = None, dual_hand_state_array = None,
        ...
        while True:
            print("[DEBUG] waiting DDS, right =", np.array(self.right_hand_state_array[:]))
        ...
        self.control_thread = threading.Thread(target=self.control_process, args=(left_hand_array, right_hand_array,  self.left_hand_state_array, self.right_hand_state_array,
    ...
    def ctrl_dual_hand(self, left_q_target, right_q_target):
        ...
        hand_msg = MotorCmds_()
        hand_msg.cmds = [unitree_go_msg_dds__MotorCmd_()
                        for _ in range(Inspire_Num_Motors * 2)]

        for idx, id in enumerate(Inspire_Left_Hand_JointIndex):
            hand_msg.cmds[id].q = left_q_target[idx]

        for idx, id in enumerate(Inspire_Right_Hand_JointIndex):
            hand_msg.cmds[id].q = right_q_target[idx]
    ...
    def control_process(self, left_hand_array, right_hand_array, left_hand_state_array, right_hand_state_array,
        ...
        try:
            while self.running:
                ...
                def is_valid_hand(x):
                    return np.linalg.norm(x) > 1e-6

                right_valid = np.any(np.abs(right_hand_data) > 1e-6)
                left_valid = np.any(np.abs(left_hand_data) > 1e-6)

                if right_valid and left_valid: # if hand data has been initialized.
...
class Inspire_Controller_FTP:
    ...
    def _send_hand_command(self, left_angle_cmd_scaled, right_angle_cmd_scaled):
        ...
        if self._debug_count < 50:
            ...
            self._debug_count += 1
        logger_mp.info(f"SEND cmd first value = {left_angle_cmd_scaled[0]}")
